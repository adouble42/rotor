/*
 * The blake256_* functions are largely copied from
 * blake256_light.c from the BLAKE website:
 *
 *     http://131002.net/blake/
 *
 * The hmac_* functions implement HMAC-BLAKE-256
 * HMAC is specified by RFC 2104.
 * PBKDF2 support added 2016 mrn@sdf.org. 224 dropped.
 */
...
void blake256_update(state *S, const void *data, uint64_t datalen) {
    int left = S->buflen >> 3;
    int fill = 64 - left;
...
void blake256_hash(uint8_t *out, const void *in, uint64_t inlen) {
    state S;
    blake256_init(&S);
...
void hmac_blake256_init(hmac_state *S, const void *_key, size_t keylen) {
...
// datalen = number of bits
void hmac_blake256_update(hmac_state *S, const void *data, size_t datalen) {
  // update the inner state
  blake256_update(&S->inner, data, datalen);
}

void hmac_blake256_final(unsigned char digest[32], hmac_state *S) {
    uint8_t ihash[32];
...
void hmac_blake256_hash(uint8_t *out, const void *key, uint64_t keylen, const uint8_t *in, uint64_t inlen) {
    hmac_state S;
    hmac_blake256_init(&S, key, keylen);
    hmac_blake256_update(&S, in, inlen * 8);
    hmac_blake256_final(out, &S);
}
 ...
 */
void
PBKDF2_BLAKE256(const uint8_t * passwd, size_t passwdlen, const uint8_t * salt,
